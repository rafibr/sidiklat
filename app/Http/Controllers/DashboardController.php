...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $progressPegawai = Pegawai::select(
            'pegawais.id',
            'pegawais.nama_lengkap as nama',
            'pegawais.jabatan',
            'pegawais.unit_kerja'
        )
            ...
            ->groupBy('pegawais.id', 'pegawais.nama_lengkap', 'pegawais.jabatan', 'pegawais.unit_kerja')
            ->orderByDesc('jp_tercapai')
            ->limit(10)
            ->get();

        // Add yearly JP targets and calculate percentage to each pegawai in the progress list
        $progressPegawai->transform(function ($pegawai) use ($year) {
            $pegawai->jp_target = $pegawai->getJpTargetForYear($year);
            $pegawai->persentase = $pegawai->jp_target > 0
                ? round(($pegawai->jp_tercapai / $pegawai->jp_target) * 100, 1)
                : 0;
            return $pegawai;
        });
