						...
						<table class="min-w-full text-sm">
							<thead class="bg-gradient-to-r from-gray-50 to-gray-100">
								<tr>
									...
									<th
										class="px-4 lg:px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
										Target {{ selectedYear }}</th>
									<th
										class="px-4 lg:px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
										Progress {{ selectedYear }}</th>
									<th
										class="px-4 lg:px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
										Semua Tahun</th>
								</tr>
							...
							<tbody class="bg-white divide-y divide-gray-200">
								<tr v-for="pegawai in progressPegawai" :key="pegawai.id"
									...
									<td class="px-4 lg:px-6 py-4">
										<div class="space-y-1">
											<div v-for="(yearData, year) in pegawai.yearly_progress" :key="year"
												class="flex items-center gap-2 text-xs">
												<span class="font-medium text-gray-600 w-12">{{ year }}:</span>
												<div class="flex-1 bg-gray-200 rounded-full h-2">
													<div class="h-2 rounded-full transition-all"
														:class="yearData.progress_percentage >= 100 ? 'bg-green-500' : yearData.progress_percentage >= 75 ? 'bg-blue-500' : yearData.progress_percentage >= 50 ? 'bg-yellow-500' : 'bg-red-500'"
														:style="{ width: Math.min(100, yearData.progress_percentage || 0) + '%' }">
													</div>
												</div>
												<span class="text-gray-700 font-semibold w-10 text-right">{{
													yearData.progress_percentage }}%</span>
											</div>
										</div>
									</td>
								</tr>
