    ...
    <AppLayout>
        <div class="relative p-3 sm:p-4 md:p-6 animate-slide-up">
            ...
            <div class="bg-gray-50 p-3 sm:p-4 rounded-lg mb-4 md:mb-6 animate-slide-left delay-100">
                ...
                <div class="mb-2">
                    <label class="text-sm font-medium text-gray-600 mb-2 block">Pilih tahun untuk ditambahkan:</label>
                    <div class="flex flex-wrap gap-2">
                        <button v-for="year in availableYearsToAdd" :key="year" @click="addYearFromPill(year)"
                            class="inline-flex items-center px-3 py-2 rounded-full text-sm font-medium bg-gray-200 text-gray-700 hover:bg-gray-300 hover:text-gray-900 transition-colors duration-200">
                            <i class="fas fa-plus text-xs mr-1"></i>
                            {{ year }}
                        </button>
                        <span v-if="availableYearsToAdd.length === 0" class="text-sm text-gray-500 py-2">
                            Semua tahun sudah dipilih
                        </span>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        addYearFromPill(year) {
            if (!this.selectedYears.includes(year)) {
                const newYears = [...this.selectedYears, year];
