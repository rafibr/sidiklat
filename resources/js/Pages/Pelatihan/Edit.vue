      ...
      <form @submit.prevent="submit" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
          ...
          <select
            ...
            :class="{ 'border-red-500': form.errors.pegawai_id }"
            required
          >
          ...
          <p v-if="form.errors.pegawai_id" class="mt-1 text-sm text-red-600">{{ form.errors.pegawai_id }}</p>
        </div>
        ...
        <div>
          ...
          <input
            type="text"
            v-model="form.nama_pelatihan"
            id="nama_pelatihan"
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            :class="{ 'border-red-500': form.errors.nama_pelatihan }"
            required
          />
          <p v-if="form.errors.nama_pelatihan" class="mt-1 text-sm text-red-600">{{ form.errors.nama_pelatihan }}</p>
        </div>

        <!-- Jenis Pelatihan -->
        <div>
          <label for="jenis_pelatihan" class="block text-sm font-medium text-gray-700 mb-2">Jenis Pelatihan</label>
          <select
            v-model="form.jenis_pelatihan"
            id="jenis_pelatihan"
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            :class="{ 'border-red-500': form.errors.jenis_pelatihan }"
            required
          >
            <option value="">Pilih Jenis Pelatihan</option>
            <option v-for="jenis in jenisPelatihan" :key="jenis" :value="jenis">{{ jenis }}</option>
          </select>
          <p v-if="form.errors.jenis_pelatihan" class="mt-1 text-sm text-red-600">{{ form.errors.jenis_pelatihan }}</p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            id="penyelenggara"
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            :class="{ 'border-red-500': form.errors.penyelenggara }"
            required
          />
          <p v-if="form.errors.penyelenggara" class="mt-1 text-sm text-red-600">{{ form.errors.penyelenggara }}</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              min="1"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{ 'border-red-500': form.errors.jp }"
              required
            />
        ...
        <div>
          ...
          ></textarea>
          <p v-if="form.errors.deskripsi" class="mt-1 text-sm text-red-600">{{ form.errors.deskripsi }}</p>
        </div>
        ...
        <div>
          ...
          <div v-if="pelatihan.sertifikat_path" class="mb-2">
            ...
            <a :href="`/storage/${pelatihan.sertifikat_path}`" target="_blank" class="text-blue-600 hover:text-blue-800">
          ...
          <input
            ...
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            :class="{ 'border-red-500': form.errors.sertifikat }"
          />
          <p class="mt-1 text-xs text-gray-500">Format: PDF, JPG, PNG. Maksimal 2MB. Kosongkan jika tidak ingin mengubah.</p>
          <p v-if="form.errors.sertifikat" class="mt-1 text-sm text-red-600">{{ form.errors.sertifikat }}</p>
        </div>
...
export default {
  ...
  setup(props) {
    const form = useForm({
      ...
      jenis_pelatihan: props.pelatihan.jenis_pelatihan,
